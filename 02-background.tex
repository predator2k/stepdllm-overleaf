...
Discrete diffusion language models (dLLMs) generate text by iteratively unmasking a fully corrupted sequence over $T$ denoising steps. Let $\mathcal{V}$ denote the vocabulary and $L$ the total sequence length comprising $m$ prompt tokens and $L-m$ response tokens at positions $\mathcal{R}=\{m{+}1,\ldots,L\}$. Starting from $x^{(0)}$ in which all response positions are set to $[\mathrm{MASK}]$, each step $s$ applies a Transformer denoiser $f_\theta$ to produce per-token distributions $\pi_i^{(s)}=\mathrm{Softmax}(f_\theta(x^{(s)})_i)$ for $i\in\mathcal{R}$. A subset of currently masked positions is then unmasked by sampling from $\pi_i^{(s)}$ according to a decreasing mask-ratio schedule, while prompt tokens remain frozen throughout. After $T$ steps all masks are removed and $x^{(T)}$ is returned.
...
Classical sparse attention imposes fixed patterns---strided, sliding-window, or global-token masks---achieving sub-quadratic complexity~\citep{child2019sparse,beltagy2020longformer,zaheer2020bigbird} at the cost of input adaptivity.
Dynamic methods instead build block-sparse masks on the fly via online index estimation, query-aware selection, or learned gating~\citep{jiang2024minference,tang2024quest,zhu2025tactic,gao2024seerattention,yuan2025nsa}, with kernel-level advances~\citep{dao2022flashattention,dao2023flashattention2,ye2025flashinfer,xu2025xattention} translating these savings into wall-clock speedups.
All of the above assume \emph{causal} attention and recompute the mask at every forward pass.
Our work departs from this paradigm in two ways: we target the \emph{bidirectional} attention of dLLMs, and we \emph{amortize} mask computation across diffusion steps by exploiting the temporal stability unique to iterative denoising (Section~\ref{sec:mask_reuse}).
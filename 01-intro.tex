...
which is also known as block diffusion \cite{wu2025fastdllm}, which sampling applies the origin reverse
process within each block and the autoregressive sampling across blocks. This can make 
KV cache possible during the generation stage, thus reducing the computation cost. 
...
To substantiate this observation, we further analyze the evolution of attention score patterns across unmasking steps. Figure~\ref{fig:cross_step} visualizes how attention score patterns evolve across unmasking steps. 
...
These observations challenge the implicit assumption behind mask reuse---that the top-$K$ important connections identified at early steps remain top-$K$ for later steps---and motivate us to reuse sparse patterns locally while refreshing the mask at selected anchor steps along the denoising trajectory.
...
\begin{figure}[t] % 去掉 *，使其回归单栏
    ...
    \begin{subfigure}[b]{0.49\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/attention_multi_position.png}
        \caption{Step similarities \red{font size}}
...
Motivated by the step-dependent drift of attention patterns, we propose \textbf{Step-dLLM}, a sparse attention framework tailored for dLLMs. Instead of reusing a one-shot sparse pattern throughout the entire denoising process, Step-dLLM refreshes the sparse attention matrix at selected anchor steps and reuses it only within locally consistent step regions, thereby better matching the evolving attention structure while avoiding the overhead of per-step reconstruction. We also observe mild head-wise heterogeneity in sparse attention, where different heads may exhibit different concentration patterns. This further suggests that the sparse budget should be allocated across heads more flexibly, rather than using a uniform rule. Based on these two observations, Step-dLLM combines anchor-step sparse mask updates with a global budget allocation strategy across heads, and is further supported by an efficient sparse-attention kernel for practical acceleration.
...
\begin{itemize}
    ...
    % quality.
    \item we present an empirical analysis of sparse attention in diffusion LLMs and identify two key properties: the sparse attention pattern is not globally reusable across denoising steps, but only locally consistent, and different heads exhibit different concentration behaviors. These observations reveal the limitations of one-shot mask reuse and uniform token-level sparsification in prior sparse DLLM methods. 
    
    \item We propose \textbf{Step-dLLM}, a sparse attention framework tailored for dLLMs, which refreshes the sparse attention matrix at selected anchor steps and allocates sparse computation globally across heads. 
    
    % \item We further introduce a \textbf{global budget allocation strategy across heads}, which allocates sparse computation according to head-specific concentration patterns instead of enforcing a uniform sparsity ratio across heads.
    
    \item We design an efficient customized sparse-attention kernel and conduct extensive experiments on representative diffusion LLMs, showing that Step-dLLM achieves a better accuracy--efficiency trade-off than prior sparse baselines, especially under high sparsity. 

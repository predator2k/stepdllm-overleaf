...
All experiments use a single NVIDIA H100 GPU with batch size 1. For Step-dLLM and SparseD, we set block size $b = 16$, block length $L = 32$, and refresh frequency $f = 0.2$. Sparsity budgets range from 50\% to 95\% for short-context tasks and 20\% to 95\% for long-context tasks. For SparseD, we use \(\text{skip}\% = 5\%\) of initial dense steps. Fast-dLLM uses block length $L = 32$ and confidence threshold $0.9$. StreamingLLM uses $s_0 = 10\%$ sink tokens and window size $W = (\rho - s_0) \cdot L_{\text{total}}$. We set $L_{\text{total}} = N_{\text{steps}}$ for all methods. Full details are in the Appendix.
...
\begin{figure}[t]
    ...
    \caption{Kernel-level wall-clock speedup of the fused block-sparse attention kernel over the dense Flash Attention baseline. Speedup increases with both sequence length and sparsity ratio, reaching $8.6\times$ at 16K tokens with 95\% sparsity. Points follow tabulated values in \texttt{plot\_kernel\_speedup.py} (documented in repository; re-measure on target GPU for camera-ready claims).}
    \label{fig:kernel}
\end{figure}
